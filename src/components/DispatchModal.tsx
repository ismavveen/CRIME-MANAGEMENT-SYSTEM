...
import { MapPin, Clock, User, AlertTriangle, Shield, FileText, Image, Video, Users, Phone, Mail, Calendar, Target, Zap, Play, Eye } from 'lucide-react';
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              {report.images.map((image: string, index: number) => (
                                <div 
                                  ...
                                  onClick={() => handleViewMedia(image, 'image')}
                                >
                                  <img 
                                    src={image} 
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                              {report.videos.map((video: string, index: number) => (
                                <div 
                                  key={index}
                                  className="relative bg-gray-800 rounded-lg overflow-hidden cursor-pointer hover:bg-gray-700 transition-colors group"
                                  onClick={() => handleViewMedia(video, 'video')}
                                >
                                  <video 
                                    src={video}
                                    className="w-full h-24 object-cover"
