...
import { GoogleAuth, JWTInput } from "https://esm.sh/google-auth-library@9.9.0";
...
serve(async (req) => {
  ...
  try {
    // Parse form
    const { email, fullName, rank, unit, category } = await req.json();

    if (!email || !fullName || !rank) {
      return new Response(
    ...
    const portalLink = `${Deno.env.get("SITE_URL") || "http://localhost:5173"}/commander-portal`;
    ...
    const emailHTML = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Defense Headquarters - Account Created</title>
        <style>
          ...
          .login-link-box { background: #f5f5fc; border-left: 5px solid #0f72e5; padding: 22px 20px; margin: 20px 0 30px 0; border-radius: 6px; }
          .login-link-box a { display: block; background: #002b5c; color: #fff !important; text-decoration: none; font-size: 1.13rem; font-weight: 600; margin: 18px 0 13px 0; padding: 13px 20px; border-radius: 6px; letter-spacing: 0.5px;}
          .security-reminders { font-size: 0.99rem; color: #9a5001; background: #fff3cd; border-left: 5px solid #ffbb33; margin: 16px 0; padding: 11px 14px; border-radius: 3px; }
        ...
        <div class="container">
          ...
          <div class="content">
            <div class="important">
              ...
              <span>
                Your Commander account for <b>${unit ? `${unit}, ` : ""}${category}</b> has been successfully created.
              </span>
            </div>
            <div class="login-link-box">
              <span style="font-size:1.07rem"><b>✅ Your Account is Active</b></span><br/>
              You can now log in to the Unit Commander Portal using the credentials you created.<br/>
              <a href="${portalLink}">Login to Commander Portal</a>
            </div>
            <div class="security-reminders">
              - Keep your password secure and do NOT share it.<br/>
              - For security, we will never email you your password.
            </div>
            <div class="footer">
    ...
    const message =
      ...
      `To: ${email}\r\n` +
      `Subject: ✅ Your Unit Commander Portal Account is Ready\r\n` +
      "MIME-Version: 1.0\r\n" +
    ...
    console.error("Error in send-registration-complete-email function:", error);
    return new Response(
